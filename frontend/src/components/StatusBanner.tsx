...
export function StatusBanner({ status }: StatusBannerProps) {
  ...
  return (
    <div
      // Errors are interruptive — screen readers should announce immediately.
      // Info/warning/success stay polite.
    ...
    >
      <Icon aria-hidden="true" className="h-4 w-4 mt-0.5 flex-shrink-0" />
      <span className="leading-snug">
        <span className="sr-only">{toneSrPrefix[status.tone]} </span>
        {status.message}
      </span>
    </div>
  )
